...
SQLite3::Database.open("appeng_take_home_db") do |db|
  ...
  db.execute("SELECT team_id, COUNT(*) AS NUM_OF_USERS_IN_GROUP FROM users WHERE team_id IN #{current_teams_str} GROUP BY team_id ORDER BY NUM_OF_USERS_IN_GROUP DESC") do |team_obj|
